...
namespace SchoolProject.XUnitTest.ServicesTest.ExtensionMethod
{
    ...
    {
        private readonly Mock<IPaginatedService<Student>> paginatedServiceMock;
        public ExtensionMethodTest()
        {
            paginatedServiceMock = new();
        }
        ...
        public async Task ToPaginatedListAsync_Should_Return_List(int pageNumber, int pageSize)
        {
            ...
            paginatedServiceMock.Setup(x => x.ReturnPaginatedResult(studentList, pageNumber, pageSize)).Returns(Task.FromResult(paginatedResult));
            //Act
            var result = await paginatedServiceMock.Object.ReturnPaginatedResult(studentList, pageNumber, pageSize);
